...
import {
  ...
  useUpdateSiteType,
  useDeleteSiteType,
} from "@/features/site/hooks/useSiteTypes";
...
function TypeListItem({
  ...
  type: SiteType;
  isEditing: boolean;
  onEdit: (t: SiteType) => void;
}) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const { mutate: del, isPending } = useDeleteSiteType();
...
interface DutySiteTypeModalProps {
  open: boolean;
  onClose: () => void;
}

export function DutySiteTypeModal({ open, onClose }: DutySiteTypeModalProps) {
  const { data, isLoading } = useSiteTypeList();
  const { mutate: createType, isPending: creating } = useCreateSiteType();
  const { mutate: updateType, isPending: updating } = useUpdateSiteType();

  const types: SiteType[] = data?.data?.siteTypes ?? [];

  const [editTarget, setEditTarget] = useState<SiteType | null>(null);
  ...
  const handleEdit = (type: SiteType) => {
